@props(['job'])
<div class="bg-gray-300 rounded-md p-4 flex">
    <img class="w-28 aspect-square rounded-lg" src="{{ asset($job->company->logo_path) }}" alt="Company Photo">

    <div class="ml-3 flex flex-col flex-grow">
        <h2 class="text-lg -my-0.5 font-semibold">
            <a href="{{ $job->url }}" target="_blank">{{ $job->company->name }}</a>
        </h2>
        <p class="text-md">{{ $job->position }}</p>
        <p class="text-md">{{ $job->salary }}</p>
    </div>
    <div class="flex items-end">
        <button class="hover:underline" onclick="openInfoModal({{ json_encode($job) }})">Click here to see more</button>
    </div>
</div>
